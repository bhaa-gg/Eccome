...
export default function User() {
    ...
    function updatePassss(values) {
        axios.put("https://ecommerce.routemisr.com/api/v1/users/changeMyPassword", values, {
            ...
            console.log(err);
            setMessagesPass(err.response.data.message)
    ...
    function updateDataa(values) {
        axios.put("https://ecommerce.routemisr.com/api/v1/users/updateMe", values, {
            ...
            console.log(err);
            setMessagesUpdate(err.response.data.message)
        })
    }
            ...
            <Modal keyboard={false} size='lg' centered backdrop="static" show={show2} onHide={handleClose2}>
                ...
                <Modal.Body>
                    <form onSubmit={dataUpdate.handleSubmit}>
                        ...
                        <input onBlur={dataUpdate.handleBlur} onChange={dataUpdate.handleChange} placeholder='Name...' type="text" id='name' name='name' className={` mb-4 form-control ${dataUpdate.errors.name && dataUpdate.touched.name ? "is-invalid" : ""} `} />
                        ...
                        <input onChange={dataUpdate.handleChange} placeholder='Email...' onBlur={dataUpdate.handleBlur} type="email" id='email' name='email' className={`  mb-4 form-control ${dataUpdate.errors.email && dataUpdate.touched.email ? "is-invalid" : ""}  `} />
                        {dataUpdate.errors.email && dataUpdate.touched.email ? <div className="alert alert-danger">{dataUpdate.errors.email}</div> : ""}

                        {messagesUpdate ? <div className="alert alert-danger">{messagesUpdate}</div> : ""}
                    </form>
